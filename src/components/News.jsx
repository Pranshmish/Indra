...
export default function News() {
    ...
    const getCachedNewsData = () => {
        const cachedData = localStorage.getItem('weather_news_data');
        const cachedTime = localStorage.getItem('weather_news_timestamp');

        if(cachedTime && cachedData && isCacheValid(Number(cachedTime))){
    ...
    useEffect(() => {
        const getNews = async () => {
            try {
                const gotNewsFromCache = getCachedNewsData();
                if(gotNewsFromCache){
                    setNews(gotNewsFromCache);
                    setloading(false);
                    console.log('Cache se data mil gya');
                    return;
                }
                ...
                localStorage.setItem('weather_news_data',JSON.stringify(newsData.results || []));
                localStorage.setItem('weather_news_timestamp',Date.now().toString());
